...
本文给出「寻迹」校园失物招领平台的整体架构说明。课程项目围绕两个 Vue 前端、
FastAPI 主后端、独立 AI 服务、MySQL、MinIO 与容器化部署形成完整技术体系，
覆盖系统边界、模块职责、数据组织、核心流程、关键决策与部署方案。架构设计以
校园失物招领业务闭环为中心，同时兼顾可维护性、可靠性、安全性和持续演进能力。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{3.0cm}Y Y}
    ...
    工程实现 & 分层模块、DTO、持久任务与安全机制 & 低耦合、可测试和可维护 \\
...
「发布---检索与匹配---分级验证---双确认交接---归档与信用记录」。课程项目版本优先
保证可演示、可测试、可部署和可追踪，而不是追求互联网规模。主要架构驱动如下：
...
\begin{itemize}
  ...
  \item 课程交付覆盖需求、接口、实现、测试和部署的完整软件工程过程。
\end{itemize}
...
课程项目将学校统一身份、商业短信、支付、物流和实时聊天定义为外部扩展边界。
当前版本采用受白名单约束的验证码演示路径，并把 AI 模型设计为可替换增强能力，
保证基础发布和认领流程始终独立、稳定。
...
默认 Compose 将 MySQL 与 MinIO 端口绑定到 \term{127.0.0.1}，生产覆盖文件进一步移除
二者的宿主机端口。后端容器启动命令先执行 \term{alembic upgrade head}，再启动 Uvicorn。
两个前端通过相对 API 路径复用同源代理，减少浏览器跨域配置。健康检查目前主要证明
进程可响应，并不等价于数据库、对象存储和业务就绪。
...
设计约束规定每个业务包采用 \path{router/service/repository/models/schemas/deps}：router
负责 HTTP、依赖和 Schema；service 负责状态守卫、事务和跨域编排；repository 只负责
SQLAlchemy 查询与写入；ORM 与 Pydantic DTO 分离。当前所有业务 router 均通过 deps
获得 service，未直接导入 repository 或 DB session，这是已实现的清晰边界。

跨模块协作由 service 统一编排，\path{user/service.py}、\path{item/service.py}、
\path{match/service.py} 负责组合相邻领域能力；repository 和 model 只承担明确的数据访问
职责。该组织方式在保持模块内聚的同时，支持匹配、认领和交接等跨域事务完整落地。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{3.5cm}Y Y}
    ...
    认领与交接 & \texttt{claim\_requests}、\texttt{claim\_answers}、\texttt{handover\_records} & handover 的 claim 唯一；回答保留题目快照 \\
...
数据库结构以 Alembic 为唯一演进链，当前单一 head 为
\texttt{20260711\_0008}。迁移不仅创建表，还包含历史数据收敛：0004 在增加 match pair
唯一约束前重写 claim 和 notification 引用并删除重复行；0007 在举报唯一约束前清理重复；
0008 增加带检查约束、领取索引和版本唯一键的持久任务表。部署不再挂载历史手写
\path{backend/sql/schema.sql}，旧库必须先结构比对再决定迁移或 stamp。

业务关系采用逻辑 ID 和应用层守卫，在保持课程迭代低耦合的同时，由 service 统一维护
引用完整性；部署过程通过数据核对、明确删除策略和逐步增加外键持续强化数据库约束。
普通枚举主要由 Schema/service 校验，同时
\texttt{durable\_jobs} 对类型和状态设置了数据库检查约束。

\section{关键时序：发布、任务与匹配}

图~\ref{fig:publish-sequence} 对应已落地的可靠发布路径。接口成功只表示物品与任务已经
提交，不表示 AI 已完成；分类失败会重试，匹配在 AI 不可用时可直接使用规则评分，
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[state,text width=2.7cm] (persist) at (4,0) {7. 原子写回\\匹配/通知/任务完成};
...
综合评分权重为图像 0.4、文本 0.3、地点 0.2、时间 0.1，但只在「双方均有输入且该
维度确有可用评分器」的维度上归一。主后端不信任远端 \texttt{totalScore}，会校验所有
分值、来源、可用性并重新计算。课程版本以文本、地点与时间评分为稳定基线，图像维度通过
\texttt{imageAvailable=false} 显式关闭；规则版使用中英文词元 Jaccard、地点相等/
包含关系和失物时间区间距离。总分低于 70 不保留活动匹配，达到 90 且候选为证件时发
高优先级通知。数据库同时保存 \texttt{degraded} 与 \texttt{scoreSource}，使前端和运营
可以区分模型增强与规则结果。
...
\term{AsyncSession} 编排多表写入并在末尾统一 commit。发布流程在一次提交中写物品、
图片、操作日志和 job；任务执行在一次提交中写匹配、通知和任务完成；交接结案在一次
提交中写 claim、found、关联 lost、handover、积分、通知和日志。异常离开请求时未提交
会话由依赖生命周期关闭并回滚。

设计文档要求使用显式 \term{async with session.begin()}，但现有 service 大多依赖
SQLAlchemy 的 autobegin 后统一 \term{commit()}。其原子性来自同一会话的一次提交，
代码形态与书面硬约束并不完全一致；后续应统一事务模板并增加失败注入测试。
...
核心状态以 \path{docs/architecture/enums.md} 为唯一字典：lost 从 SEARCHING 到 FOUND 或 CLOSED；
...
service 校验角色、所有权和源状态，前端只展示后端返回的能力和结果。

\section{AI 隔离、降级与可靠性}

AI 服务是独立进程，通过 \path{/internal/ai/*} 提供分类、敏感检测和匹配评分。主后端
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{2.8cm}Y Y}
    ...
    课程演示 & 复用已演练联调环境，冻结代码、迁移和演示数据 & 答辩前不临时改表；保留规则降级和固定脚本 \\
    扩展部署 & HTTPS 网关，仅公开前端/443，内部化 backend/AI/MinIO & 可按实际运行环境继续增强 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{longtable}{L{1.4cm} L{3.5cm} L{5.4cm} L{4.1cm}}
  ...
  \midrule
  \endhead
  001 & 主后端采用模块化单体，AI 独立部署 & 控制五人团队的联调、配置和排错成本，同时隔离模型依赖 & 业务模块共享数据库；团队/流量扩大后再按边界拆分 \\
  002 & 主后端与 AI 统一 Python async 栈 & FastAPI/Pydantic 契约清晰，httpx 与异步 DB 适合外部调用 & CPU/阻塞任务需线程池或独立 worker \\
  003 & MySQL + 私有 MinIO，库中只存稳定引用 & 结构化数据与大对象分离，可动态授权和缩短敏感 URL & 需要资产绑定、孤儿清理和对象权限运维 \\
  004 & 数据库 outbox 替代仅内存 BackgroundTasks & 发布与任务原子落库，进程重启可恢复；无需引入消息中间件 & runner 与应用同进程，规模扩大应迁移独立 worker/队列 \\
  005 & AI 可降级，敏感判断 fail-closed & 模型不可用不阻断主流程，又不以不确定结果换取公开展示 & 结果质量受规则限制，需记录来源并支持人工复核 \\
  006 & 双 Vue SPA 共享 TypeScript 类型 & 用户端移动优先、管理端桌面任务密集，复用枚举和契约 & 共享包目前是源码别名，需加强契约自动生成 \\
  007 & ULID 与统一枚举 & 跨模块 ID 一致、可排序；避免中文状态和魔法值扩散 & 持续增强数据库与接口枚举一致性校验 \\
  \bottomrule
\end{longtable}

\section{课程目标完成与扩展路线}

本项目已经完成课程范围内的整体架构目标：双前端覆盖用户与管理场景，模块化主后端承载
核心业务，独立 AI 服务提供智能增强，MySQL 与 MinIO 分别管理结构化数据和文件资产，
持久任务机制串联分类、敏感检测、匹配与通知，Docker Compose 和持续集成支撑统一交付。

在课程成果基础上，系统可沿以下方向继续扩展：

\begin{enumerate}
  \item 引入校园统一身份与商业短信服务，进一步丰富真实校园环境的身份接入方式；
  \item 增加图像向量、候选召回索引与批量评分，提升大规模物品匹配效率；
  \item 将 runner 扩展为独立 worker，并结合队列、指标和管理页面强化任务运营；
  \item 增加统一 HTTPS 网关、集中观测、备份恢复和不可变镜像，支持更高等级部署；
  \item 持续自动生成接口契约和架构约束检查，保持文档、枚举与实现同步演进。
\end{enumerate}
...
系统实现了「发布不被 AI 阻塞、状态变化可追踪、多实体写入统一提交、敏感结果分层展示」
四项核心原则，完整支撑校园失物招领从发布到交接归档的课程项目目标。
