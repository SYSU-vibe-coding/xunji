...
async def _seed_owned_match(session: AsyncSession) -> tuple[LostItem, FoundItem, MatchResult]:
    ...
    _image(
        ...
        biz_type="FOUND",
        biz_id=found.id,
        url="http://127.0.0.1:9000/xunji/FOUND/legacy/found.jpg",
    )
...
async def test_list_matches_filters_and_uses_camel_case(
    ...
    assert item["imageAvailable"] is False
    assert item["scoreSource"] == "RULE_BASED"
    ...
    assert item["counterpart"]["coverImageUrl"].startswith("https://signed.test/xunji/")
...
async def test_recalculate_invokes_trigger_match(
    ...
    async def fake_trigger_match(biz_type: str, item_id: str, **kwargs: Any) -> int:
        calls.append((biz_type, item_id, kwargs.get("max_candidates")))
        return 3
    ...
    body = resp.json()
    assert body["code"] == 0
    assert body["data"]["matchedCount"] == 3
    assert body["data"]["status"] == "COMPLETED"
    assert "taskId" not in body["data"]
    assert calls == [("LOST", lost.id, 100)]
...
async def test_biz_type_and_id_must_be_provided_together(
    client: AsyncClient,
    auth_headers: dict[str, str],
) -> None:
    only_type = await client.get(
        "/api/v1/matches", params={"bizType": "LOST"}, headers=auth_headers
    )
    only_id = await client.get(
        "/api/v1/matches",
        params={"bizId": "01ARZ3NDEKTSV4RRFFQ69G5FAV"},
        headers=auth_headers,
    )

    assert only_type.json()["code"] == 40001
    assert only_id.json()["code"] == 40001


async def test_list_my_matches_is_not_limited_to_first_ten_user_items(
    client: AsyncClient,
    session: AsyncSession,
    auth_headers: dict[str, str],
) -> None:
    ...
    for index in range(11):
        lost = _lost_item(
            ...
            name=f"Owned item {index}",
        )
        _match(session, lost_id=lost.id, found_id=found.id, total_score="80.00")
    await session.commit()
