...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \node[umlclass] (claim) at (4.5,-8.0) {\centering\textbf{ClaimRequest}\nodepart{second}
      id, matchId?, foundItemId\\claimantId, verifyLevel\\reviewStatus, verifySource};
    \node[umlclass] (answer) at (1.6,-11.8) {\centering\textbf{ClaimAnswer}\nodepart{second}
      id, claimId, questionId\\question/reference snapshots\\answerText, matchScore};
...
\begin{longtable}{L{2.6cm} L{3.0cm} L{8.4cm}}
  ...
  ClaimRequest & 回答、凭证图片、唯一交接 & found 是占用锁根；问题和参考答案按提交时快照；校验来源可审计；审核、申诉和交接状态必须与 found/match 同步 \\
...
AI 服务返回内容需要满足约定的数据结构，总分按照可用维度统一归一。图像维度不可用时只采用文本、地点、时间和结构化属性；认领问答使用小模型比较原始回答与参考答案，模型不可用时回退到关键词规则并记录来源。辅助服务故障不得阻断物品发布，敏感检测不能以不确定结果解除保护，证件类别始终按敏感物品处理。
...
状态不是固定从 \term{PENDING} 开始，而是由创建时验证结果直接决定。\term{LEVEL\_1} 语义问答通过进入 \term{ANSWER\_PASSED}；\term{LEVEL\_2} 问答通过但缺凭证进入 \term{PROOF\_PENDING}，已有凭证则进入 \term{ANSWER\_PASSED}；\term{LEVEL\_3} 缺凭证进入 \term{PROOF\_PENDING}，已有凭证仍需人工，进入 \term{PENDING}。一级/二级问答失败直接保存带问题、参考答案、回答、分数和校验来源快照的 \term{REJECTED} 记录，并只向认领者返回统一失败消息。
...
\begin{longtable}{L{2.1cm} L{3.3cm} L{5.2cm} L{3.4cm}}
  ...
  \midrule
  \endhead
  发起认领 & found=PENDING 且审核通过；无活动认领；非本人招领；用户信誉合格 & 有效验证时 found$\rightarrow$CLAIMING，创建 claim；合法 match$\rightarrow$CLAIMED & 保存问答与来源快照；通知拾获者、写日志；失败问答不占用 found/match \\
