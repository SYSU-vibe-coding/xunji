...
- `LEVEL_1` 判定已有问答，AI 语义平均分 `>=60` 后置 `ANSWER_PASSED`
- `LEVEL_2` 要求已有问答通过和凭证；缺凭证为 `PROOF_PENDING`，齐备后为 `ANSWER_PASSED`
- `LEVEL_3` 要求凭证并保持人工审核路径；缺凭证为 `PROOF_PENDING`，齐备后为 `PENDING`
- 问答允许同义词、口语化表达、语序变化和轻微错字；模型不可用时自动回退到关键词命中规则
- 问答失败写入通用失败记录并返回 `44004`，5 分钟冷却，滚动 24 小时最多 3 次；错误、拒绝原因及普通用户详情均不泄露参考答案或逐题分数，管理员审计详情见 `admin.md`
- 已存在进行中认领 → `44001`
- 信誉 `< 30` → `45002`
