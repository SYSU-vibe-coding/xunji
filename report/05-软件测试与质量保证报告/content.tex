...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{2.7cm}L{3.1cm}Y}
    ...
    管理端 & Vue 3、管理路由与审核页面 & 管理员鉴权、认证审核、内容审核、举报处置和统计展示 \\
    主后端 & FastAPI、SQLAlchemy、Alembic & 接口契约、业务规则、事务状态、权限、对象访问与异步任务 \\
    AI 服务 & 分类、敏感检测、匹配评分 & 请求鉴权、输入校验、结果范围、异常处理和规则降级 \\
...
\begin{longtable}{L{2.5cm}L{3.6cm}L{3.9cm}L{3.9cm}}
  ...
  \shortstack[l]{TC-CLAIM-01\\认领验证通过} & 失主从关联匹配发起认领并回答全部验证问题 & 校验匹配归属与答案完整性；通过后进入合法认领状态 & 归属、完整答案、审核与状态迁移检查通过 \\
  ...
  \shortstack[l]{TC-ADMIN-01\\内容审核} & 管理员查看待审核内容，填写意见并通过；随后重复提交 & 审核结果、意见与操作记录一致；过期操作返回状态冲突 & 审核、日志和重复操作检查通过 \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{2.8cm}C{2.2cm}L{3.5cm}Y}
    ...
    模块 & 用例数 & 结果 & 主要覆盖 \\
    \midrule
    主后端 & 200 & 全部通过，语句覆盖率 80\% & 用户、物品、匹配、认领、交接、积分、通知、治理和迁移 \\
    AI 服务 & 60 & 全部通过 & 配置、鉴权、分类、敏感检测、匹配评分和异常处理 \\
    用户端 & 17 & 全部通过 & 会话、导航、筛选参数、上传辅助和错误分类 \\
    管理端 & 15 & 全部通过 & 管理会话、列表请求、举报参数、错误分类和格式化 \\
    \midrule
    合计 & 292 & 全部通过 & 四个代码模块的自动化回归集合 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
本报告建立了覆盖五类测试对象、六种测试类型、18 个功能与安全核心场景及 6 个性能场景的测试方案，明确了测试环境、数据、阶段、人员、准入退出和执行方法。期末基线的四个模块共 292 个自动化用例全部通过，Backend 语句覆盖率达到 80\%，核心业务、权限和状态场景按统一预期结果完成检查。
